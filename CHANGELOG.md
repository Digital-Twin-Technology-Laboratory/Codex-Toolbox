...
- “智商显示”设置按菜单栏、榜单、变化趋势、数据刷新和综合排名权重重新分组；Token 设置也拆分为刷新、任务榜单与每日趋势。
- 各功能页底部的数据访问边界说明统一迁移到“关于 → 数据与隐私详情”。
- “展开榜单显示其他指标”对新用户默认开启，已保存的用户选择不变。

### Fixed

- Token 总量改为独占一行并自适应缩放，九位数以上不再换行。
- 四宫格榜单优先收紧并缩放完整模型名，减少 `GPT-5.6 Sol high`、`GPT-5.6 Sol xhigh` 等相近名称的不必要省略。
...
- 模型趋势支持五个固定语义颜色槽，可按现有模型家族分类分别换色对应模型、增删至最多 5 条曲线并恢复默认 Top 3；设置新增默认关闭的“固定展开”。
- 设置新增默认关闭的“展开榜单显示其他指标”；启用后，单项榜单补充另外两项原始值，综合榜单同步展示智商、费用和耗时。

### Changed

- 趋势入口出现后即在后台预热指标历史，展开使用与其他看板一致的临界阻尼弹簧；增删或更换曲线仅筛选已准备数据，不再重新解析历史。
- 趋势曲线槽改为紧凑三列，默认 3 条在同一行展示；“添加曲线”保留全部档位并用勾选标记已添加项。
- 展开榜单仍以当前排名指标为视觉主值，智商、费用、耗时等指标统一使用一次性表头，模型行只保留对齐的数值；趋势标签改为 `Sol max`、`Sol ultra`、`Sol med` 等可直接区分档位的目录简称。

### Fixed

- 修复展开“变化趋势”时，模型菜单在 SwiftUI 多次求值中反复解析完整历史、导致主线程卡死和单核 CPU 满载的问题；趋势派生数据现在仅在输入变化时于后台准备一次。
- 修复历史 Token 日期的“返回今日”按钮被 Liquid Glass 合成层遮挡、同时撑大任务卡顶部间距的问题。
...
- 额度权重在发布前再次按 2026-08-04 最新 Codex rate card 核对，更新 Sol、Luna、GPT-5.5/5.4/5.3/5.2 等标准模式的输入、缓存输入与输出信用费率，并以全局稳健样本校准；异常并发或延迟台阶不再成为单任务长期换算率。
- Usage Ledger 升级时丢弃旧原始 Token 口径及旧费率表的校准观察，避免结束窗口继续污染新模型权重估算。
...
- 趋势悬停不再每次指针移动都重新按日聚合和排序，曲线切换也不再扫描全部历史点。
- 趋势悬浮卡恢复到曲线绘图区上方并使用固定顶部安全区；1–3 条曲线逐模型单行显示，4–5 条才切换为双列，既不向上溢出也不遮挡曲线。
- 连续悬浮状态改由独立绘图子视图持有，趋势复合容器改用静态 Liquid Glass，防止下方标签和按钮跟随鼠标日期切换重绘而明暗闪动。
- 修复使用曲线筛选菜单后，菜单栏弹窗无法再通过外部点击收起的问题；关闭生命周期由状态栏控制器统一监听应用内外鼠标事件。
- 曲线替换菜单会分别标明“当前蓝色”等当前槽位和“已用于橙色”等其他槽位，修复当前模型未正确呈现选中状态、已占用模型却无原因变灰的误导反馈。
- 展开榜单的表头与数值统一按列尾端对齐，并只压缩表头模式新增的栈间距，避免普通展开场景无意义地触发滚动条。
- PKG 安装器补齐产品标题；若安装前 Codex Toolbox 正在运行，安装器在安全替换完成后会恢复启动新版本。
