...
- 新增版本化 Codex 费率清单、6 小时自动更新工作流与应用内 ETag 检查，保留历史版本以按事件时间重算 Credits。
...
- “默认展示”分段选择器在 macOS 26 及以上使用可变形 Liquid Glass 选中镜片；其他设置控件继续采用系统自动适配的新设计。
...
- 综合榜单的算法说明改为每个榜单统一显示一次，不再在每个模型下重复。
- 修复四宫格因综合榜单独有说明行、模型副标题结构不同而出现卡片高度不一致；四项榜单现在使用相同的说明和模型行结构。
