...
> [!TIP]
> **v1.3.0 正式发布候选已经完成准备。** 本版新增站长推荐、Radar 成本效率、API 等值成本、模型可见性管理和可选实验主题；版本状态与正式附件以 Releases 页面为准。
...
2. 校验文件：
   ...
   ```bash
   shasum -a 256 -c Codex-Toolbox-1.3.0-universal.pkg.sha256
   # 或
   shasum -a 256 -c Codex-Toolbox-1.3.0-universal.dmg.sha256
   ```
...
项目使用 SwiftUI、AppKit `NSStatusItem`、Swift Charts、Observation、URLSession、SQLite3 和 ServiceManagement；应用内更新使用 [Sparkle 2](https://sparkle-project.org/)。Xcode 工程由 [XcodeGen](https://github.com/yonaskolb/XcodeGen) 生成。
