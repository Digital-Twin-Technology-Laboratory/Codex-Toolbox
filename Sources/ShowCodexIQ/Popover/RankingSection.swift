...
struct RankingSection: View {
    ...
    var body: some View {
        ...
        .help(presentation == .expanded ? "" : "点击展开“\(metric.rankingTitle)”榜单")
        .accessibilityAddTraits(.isButton)
        .accessibilityAction(named: presentation == .expanded ? "收起榜单" : "展开榜单") {
            if presentation == .expanded {
                onCollapse()
            } else {
                onExpand()
    ...
    private var header: some View {
        HStack(spacing: 6) {
            ...
            if presentation == .expanded {
                ...
                .controlSize(.mini)
                .help("恢复四宫格")
                .accessibilityLabel("收起“\(metric.rankingTitle)”榜单")
